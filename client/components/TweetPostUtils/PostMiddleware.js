...
const PostMiddleware = ({
  ...
  const { currentUser, getProfileImageUrl } = useContext(TwitterContext);
      ...
      let profileImageUrl = await getProfileImageUrl(
        ...
        if (referenced_tweets.at(-1)?.type === "retweet") {
          settwitterPostData({
            ...
            superScriptUser:
              authorID === currentUser?._id ? "You" : displayName,
            ...
            public_metrics: {
              ...
              retweet_count:
                referenced_tweets.at(-1)?.id?.public_metrics?.retweet_count ||
                0,
            },
            isLiked:
